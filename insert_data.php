...
if ($conn->query($sql) === TRUE){
	...
	WE HAVE DISPLAYED YOUR INFORMATION, FOLLOWED BY THAT OF THOSE, WHO HAVE SIMILAR TRAVEL DATE TO YOU 
	AND BETWEEN SAME LOCATIONS AT THE MOMENT.
	<br/><br/>WE WILL LET YOU KNOW IF WE FIND MORE SUITABLE TRAVEL PARTNERS FOR YOU.";
}
else{
	echo "<br/><br/><br/>";
	echo "SORRY!! WE COULDN'T ENTER YOUR DATA. WE GOT THE FOLLOWING ERROR: " . $conn->error;                                                                                                                                           //AND ABS(TMEDIFF($time,time)) < 30        AND destination='$dest' AND abs(time_to_sec(timediff($time, time))) < 1800
}

//Now to display similar datas
echo "<br/><br/><br/>";

$sql = "SELECT name, trans_mode, time, email, phone, gender FROM user_info WHERE date='$date' AND source='$source' ";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "Name: " . $row["name"]. " - Travel Mode: " . $row["trans_mode"]. " - Time: " . $row["time"]. " - E-Mail: " . $row["email"]. " - Contact Number: " . $row["phone"]. " - Gender: " . $row["gender"]. "<br><br>";
    }
} else {
